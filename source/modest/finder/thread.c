...
mystatus_t modest_finder_thread_init(modest_finder_t* finder, modest_finder_thread_t* finder_thread, size_t thread_count)
{
    ...
    /* create threads */
    for(i = 0; i < finder_thread->thread->entries_size; i++) {
...
{
	size_t i;
    for(i = 1; i < finder_thread->context_list_size; i++) {
                                        ...
                                        myhtml_tree_node_t* scope_node, mycss_selectors_list_t* selector_list)
{
    ...
    /* compare results */
    while(node) {
        for(i = 0; i < finder_thread->thread->entries_length; i++)
        {
...
{
	size_t idx;
    for (idx = 0; idx < finder_thread->thread->entries_length; idx++) {
...
void modest_finder_thread_stream_single(modest_finder_thread_t* finder_thread, mycss_selectors_list_t* selector_list)
{
    ...
    while(selector_list)
    {
        for(i = 0; i < selector_list->entries_list_length; i++) {
